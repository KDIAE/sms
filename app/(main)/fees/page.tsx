...
export default function FeesPage() {
  ...
  const fetchAllStudents = useCallback(async () => {
    try {
      const res = await studentsApi.list({ limit: 500 });
  ...
  // ── Record payment handler (used by both inline popover + LogFeeDialog) ──
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="dashboard" className="mt-0">
          <div className="relative">
            ...
            {/* Show no-data overlay if API failed */}
            {!loadingDash && !dash && (
              <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-20 flex items-center justify-center rounded-lg">
                <div className="text-center">
                  <p className="text-slate-400 text-[13px] mb-2">Failed to load data. Check console for errors.</p>
                  <Button size="sm" onClick={fetchDashboard}>Retry</Button>
                </div>
              </div>
            )}
            
            {/* Always show the UI structure */}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Card className="shadow-none border-slate-200">
                  ...
                  <CardContent className="flex flex-col gap-3 relative">
                    {/* No data placeholder for breakdown */}
                    {dash && dash.total_collected === 0 && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 bg-white/60 backdrop-blur-[2px]">
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatCard label="Month Keys"         value={monthKeys.length} sub="months in session" />
      </div>
